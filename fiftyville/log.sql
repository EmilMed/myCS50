...
UPDATE flights
SET origin_airport_id = airports.full_name
FROM airports
WHERE flights.origin_airport_id = airports.id;

UPDATE flights
SET destination_airport_id = airports.full_name
FROM airports
WHERE flights.destination_airport_id = airports.id;

SELECT flights.id, flights.hour, flights.minute, airports.city, FROM flights WHERE year = 2021 AND month = 7 AND day = 29 ORDER BY hour ASC LIMIT 1;
