...
def index():
    ...
    for stock in stocks:
        ...
        stock["valueusd"] = stock["value"]
    return render_template("index.html", stocks=stocks, cash=cash, total=total_value, grand=usd(grand_total))
...
def sell():
    if request.method == "GET":
        ...
        total_shares = db.execute("SELECT SUM(shares) as shares from cashflow WHERE user_id=:id AND symbol = :symbol GROUP BY symbol", id=user_id, symbol=symbol)
